-- Safe migration: Drop existing objects first if they exist

-- Drop trigger if exists
DROP TRIGGER IF EXISTS update_game_comments_updated_at ON public.game_comments;

-- Drop function if exists (will be recreated)
...
DROP POLICY IF EXISTS "Users can delete own comments or admins can delete any" ON public.game_comments;

-- Drop table if exists (WARNING: This will delete all existing comments!)
-- If you want to keep existing data, comment out the next line
-- DROP TABLE IF EXISTS public.game_comments CASCADE;
